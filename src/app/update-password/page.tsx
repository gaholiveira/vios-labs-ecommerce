  ...
  useEffect(() => {
    ...
    async function checkSession() {
      try {
        ...
        while (attempts < maxAttempts && !sessionFound) {
          ...
          if (sessionError) {
            if (process.env.NODE_ENV === 'development') {
              console.error(`❌ Erro ao verificar sessão (tentativa ${attempts + 1}):`, sessionError);
            }
            attempts++;
          ...
          if (session) {
            ...
            if (userError || !user) {
              ...
              setError('Usuário não encontrado. Solicite um novo link de redefinição.');
              setCheckingSession(false);
              return;
            }
        ...
        if (process.env.NODE_ENV === 'development') {
          console.error('❌ Nenhuma sessão encontrada após', maxAttempts, 'tentativas');
        }
          ...
          {!success ? (
            ...
            >
              ...
              <motion.div
                ...
                className="flex justify-center mb-8"
              >
                <CheckCircle2 className="w-20 h-20 text-brand-green" strokeWidth={1.5} />
              </motion.div>
